...
def letterbox_batch(
    ...
    dw /= 2  # divide padding into 2 sides
    dh /= 2
    print("resizing : ", im[0, ...].shape)
    if shape[::-1] != new_unpad:  # resize
        resized_im = np.zeros((im.shape[0], new_unpad[1], new_unpad[0], im.shape[3]))
        for i in range(im.shape[0]):
            resized_im[i, ...] = cv2.resize(
                im[i, ...], new_unpad, interpolation=cv2.INTER_LINEAR
            )
        im = resized_im
    ...
    padded_im = np.zeros(
        (
            im.shape[0],
            im.shape[1] + top + bottom,
            im.shape[2] + left + right,
            im.shape[3],
        )
    )
    for i in range(im.shape[0]):
        padded_im[i, ...] = cv2.copyMakeBorder(
            im[i, ...], top, bottom, left, right, cv2.BORDER_CONSTANT, value=color
        )  # add border
